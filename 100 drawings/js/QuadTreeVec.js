...
/**
 * QuadTree for using with p5.Vector.
 */
class QuadTree {
  ...
  reset() {
    if (this.divided) {
      this.subregions.nw.reset(),
      this.subregions.ne.reset(),
      this.subregions.sw.reset(),
      this.subregions.se.reset()
    } else {
      this.nodes = [];
    }
    return;
  ...
  /**
   * Inspired by: https://github.com/timohausmann/quadtree-js/blob/0bb7c37cbbef838936d7c5a6507cc3589e40764c/quadtree.js
   */
	/*
  ...
  findNode = function( node ) {
        ...
        if (this.subregions.nw.findNode(node) !== false) {
          region = this.subregions.nw;
        } else if (this.subregions.ne.findNode(node) !== false) {
          region = this.subregions.ne;
        } else if (this.subregions.sw.findNode(node) !== false) {
          region = this.subregions.sw;
        } else if (this.subregions.se.findNode(node) !== false) {
          region = this.subregions.se;
        }
	...
	remove(node) {
		...
		let region = this.findNode( node ),
			index = region.nodes.indexOf( node );
